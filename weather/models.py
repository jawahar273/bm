...
class AirPollution(models.Model):
    ...
    location_lat = models.DecimalField(max_digits=14, decimal_places=4, default=0)
    location_lon = models.DecimalField(max_digits=15, decimal_places=4, default=0)

    class Meta:
        unique_together = (("location_lat", "location_lon"),)

    def __str__(self):
        return "Last Update-{:%Y-%m-%d %H:%M}" " GasType-{}".format(
            self.last_update, self.gas_type.upper()
        )


class AirPollutionData(models.Model):

    data_base = models.ForeignKey(
        "AirPollution", related_name="air_pollution_data", on_delete=models.CASCADE
    )
    ...
    def __str__(self):
        return "Date-{} Base id-{:d}, Pressure-{:.2f}" " Gas Type -{}".format(
            self.weather_date,
            self.data_base.id,
            self.pressure,
